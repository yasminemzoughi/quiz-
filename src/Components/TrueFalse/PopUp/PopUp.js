...
import './PopUp.css'; 
import { questions } from '../../TrueFalse/PopUp/Questions';
...
const PopUp = (props) => {
  ...
  return (
    <div>
      <Card className="quiz-truefalse-review">
        <h2>You scored {realScore}%</h2>
        <h2>{feedback}</h2>
        <img className="quiz-truefalse-gif" src={url} alt="gif" />
        <Button className="quiz-truefalse-restart" onClick={() => props.reset()}>
          Try again
        </Button>
      </Card>
    </div>
